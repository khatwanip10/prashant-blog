...
def admin_only(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if current_user.id != 1:
            return abort(403)
        # Other-wise continue with the route function
...
def register():
    ...
    if form.validate_on_submit():
        ...
        flash(message='You have already signed up with that email, log in instead!')
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not user:
            ...
            flash(message='Password is incorrect, please try again!')
            return redirect(url_for('login'))
...
def add_new_post():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        return redirect(url_for("get_all_posts"))
    return render_template("make-post.html", heading='New Post', form=form, logged_in=current_user.is_authenticated)
